...
- Java-based path types to safely construct `QuerySpec` objects.
...
WARNING: This annotation processor is still (highly) experimental. Feedback welcomed.

`crnk-gen-java` hosts an annotation processor that generates a type-safe API for the construction of `QuerySpec`
objects. The annotation processor can be setup like any other processor as supported by all Java build tools.
...
This leads to generated classes like:

[source,Java]
----
include::../../../../crnk-integration-examples/spring-boot-example/build/classes/java/main/io/crnk/example/springboot/domain/model/ProjectType.java[]
----

One can add a static import for `project` and make use of it:

[source,Java]
----
QuerySpec querySpec = new QuerySpec(Project.class)
querySpec.addFilterSpec(project.name.eq("John"));
----

In the future further help is expected, like a type-safe QuerySpec.
